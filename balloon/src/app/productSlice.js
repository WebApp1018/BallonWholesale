...
export const getProdcutByCategoryAsync = (category) => async (dispatch) => {
  try {
    const response = await axios.get(process.env.REACT_APP_API_BASE_URL + "api/product/get_product_by_category/" + category);
...
export const getProdcutByCodeAsync = (code) => async (dispatch) => {
  try {
    const response = await axios.get(process.env.REACT_APP_API_BASE_URL + "api/product/get_product_by_code/" + code);
...
export const { setProduct, setProductByCategory, setProductByCode } = productSlice.actions;
export const showProduct = (state) => state.product.product;
export const showProductByCategory = (state) => state.product.product_by_category;
